...
#### Copy the Cutadapt results

1. Log in to OSC at <https://ondemand.osc.edu>.
2. In the `Clusters` dropdown menu in the blue top bar, click `Pitzer Shell Access`.
3. Paste (<kbd>Ctrl/Cmd</kbd>+<kbd>V</kbd>) the following command into the shell^[
   It doesn't matter what your working dir is, this will work regardless.]:

```bash
cp -rv /fs/ess/PAS2714/share/results /fs/ess/PAS2714/users/$USER
```
```bash-out
‘/fs/ess/PAS2714/share/results/ps_fulldata/bac22rot_w_ASV.rds’ -> ‘/fs/ess/PAS2714/users/jelmer/results/ps_fulldata/bac22rot_w_ASV.rds’
‘/fs/ess/PAS2714/share/results/ps_fulldata/bac22rot_phyloseq.rds’ -> ‘/fs/ess/PAS2714/users/jelmer/results/ps_fulldata/bac22rot_phyloseq.rds’
‘/fs/ess/PAS2714/share/results/cutadapt/W404A_R2.fastq.gz’ -> ‘/fs/ess/PAS2714/users/jelmer/results/cutadapt/W404A_R2.fastq.gz’
‘/fs/ess/PAS2714/share/results/cutadapt/NW203A_R2.fastq.gz’ -> ‘/fs/ess/PAS2714/users/jelmer/results/cutadapt/NW203A_R2.fastq.gz’
‘/fs/ess/PAS2714/share/results/cutadapt/W205BC_R2.fastq.gz’ -> ‘/fs/ess/PAS2714/users/jelmer/results/cutadapt/W205BC_R2.fastq.gz’
# [...output truncated...]
```

::: {.callout-tip collapse="true"}
#### The `$USER` environment variable and the command above _(Click to expand)_

You can either use the command _exactly as is_,
or you can replace `$USER` by your username, e.g.:

```bash
cp -rv /fs/ess/PAS2714/share/results /fs/ess/PAS2714/users/jelmer
```

But do _not_ use a "hybrid approach" like `$jelmer`.
`$USER` is a so-called "environment variable" that contains your user name,
and since we're at OSC, that will be your OSC user name, specifically.
A variable like `$jelmer` (anything that starts with a `$` is a variable) will not exist
unless you assigned it.
:::
...
1.  At OnDemand, click on **`Interactive Apps`** (top bar) and then **`RStudio Server`** (all the way at the bottom).
2.  Fill out the form as follows:
    ...
    -   Node type: **`any`**
    -   Number of cores: **`4`**
3.  Click **`Launch`** and once your job has started, click **`Connect to RStudio Server`**.
...
::: callout-important
#### If you see a message like this one, do _not_ click "Install"

...but click "Don't show again".

<hr style="height:1pt; visibility:hidden;" />
![](img/rstudio_install.png){fig-align="center" width="70%"}
:::
...
We will save the number of cores in a variable that we will then use throughout this session ---
...
A phylogenetic tree can be estimated for the ASVs you inferred.
Depending on the number of ASVs recovered and the phylogenetic tree algorithm of choice,
this step could take many hours.
Simpler trees will be less computationally intensive.
Depending on the marker gene you are working on, you may or may not choose to perform this step.

This step should be conducted with the final sequence table `seqtab` after removing chimeras.
Then, the tree can be included in the *phyloseq* object, so if you have a tree,
you should rerun the `phyloseq()` function to create an object that includes it.

First, we'll need to load the
[_phangorn_](https://cran.r-project.org/web/packages/phangorn/index.html) package
for estimation of phylogenetic trees in R.
...
# Root the tree
rooted_tree <- root_by_longest_edge(fitGTR)
```
