...
We will be using all of these components during this workshop. **This session** will provide an
introduction to supercomputers in general and to the Ohio Supercomputer Center (OSC) specifically.
...
#### OSC websites _(Click to expand)_
...
we will only be working in a so-called project directory (located in `/fs/ess`)
for our workshop's OSC Project, `PAS2714`.
...
Compared to command-line computing on a laptop or desktop, a number of aspects are different when
working on a supercomputer like at OSC. We'll learn much more about these later on in the course,
...
-   **Software**\
    You generally can't install "the regular way", and a lot of installed software needs to be
    "loaded".
-   **Operating system**\
...
Hovering over the **Files** dropdown menu gives a list of directories that you have access to. If
your account is brand new, and you were added to `PAS2714`, you should only have three directories
listed:

1.  A **Home** directory (starts with `/users/`)
2.  The `PAS2714` project's "**scratch**" directory (`/fs/scratch/PAS2714`)
3.  The `PAS2714` project's "**project**" directory (`/fs/ess/PAS2714`)
...
- Interactive Apps like VS Code and RStudio **run on compute nodes** (not login nodes).
  ...
  - The "_Working Directory_" for the program:  your personal folder in `/fs/ess/PAS2714/users`.
